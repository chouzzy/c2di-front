...
import { AddEditButtons, continueSubmitInvestor, deleteUserInvestment, deleteUserInvestmentTrigger, getUserInvestments, getUsers, onSubmitInvestor } from './utils';
...
function InvestorList({ userData, projectData }: FormUsersProps) {
    ...
    useEffect(() => {
        ...
        if (!usersList) {
            ...
            getUserInvestments(setUserInvestmentsList, projectData)
        }
    ...
    useEffect(() => {
        ...
        if (deletingUserInvestment && userInvestmentID) {
            deleteUserInvestment(userInvestmentID, setUserInvestmentsList, userInvestmentsList)
            setUserInvestmentID('')
                        ...
                        <Table variant={'simple'}>
                            ...
                            <Tbody>
                                ...
                                {userInvestmentsList?.map((userInvestment, index) => {
                                    ...
                                    const userApartament = projectData.apartaments.find(apartament => apartament.id === userInvestment.apartament?.id)
                                            ...
                                            <Td>
                                                <Flex gap={2} justifyContent={'space-between'}>
                                                    ...
                                                    {editMode ?
                                                        ...
                                                        <Flex
                                                            onClick={() => {
                                                                deleteUserInvestmentTrigger({ id: userInvestment.id, setDeletingUserInvestment, setUserInvestmentID })
                                                            }}
                ...
                <AlertDialog isOpen={isOpen} leastDestructiveRef={cancelRef} onClose={onClose}>
                    <AlertDialogOverlay>
                        <AlertDialogContent>
                            ...
                            <AlertDialogBody>
                                Este apartamento já possui um usuário cadastrado. Exclua-o antes de prosseguir.
                            </AlertDialogBody>
