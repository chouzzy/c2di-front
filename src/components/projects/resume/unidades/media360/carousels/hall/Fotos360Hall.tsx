...
import { Fotos360HallInput } from "./Fotos360HallInput";
...
export function Fotos360Hall({ userData, projectData, openImage, setLoadingFiles, unidadeInView, setFotoType }: FotosGeraisProps) {

    const [editMode, setEditMode] = useState(false)
    const [isDeletingImage, setIsDeletingImage] = useState(false)
    const [deletingImageID, setDeletingImageID] = useState<string>()

    const [destaques, setDestaques] = useState(unidadeInView ? projectData.apartamentTypes[unidadeInView].media360.hall : projectData.apartamentTypes[0].media360.hall)

    useEffect(() => {

        if (unidadeInView || unidadeInView == 0) {
            setDestaques(projectData.apartamentTypes[unidadeInView].media360.hall)
        }
    ...
    useEffect(() => {
        ...
        const deleteImage = async (imageID: Investment["images"][0]["id"]) => {
            try {
                ...
                const fotoIndex = projectData.apartamentTypes[unidadeInView].media360.hall.indexOf(imageToDelete);

                // Remove a foto do array
                if (fotoIndex > -1) {
                    projectData.apartamentTypes[unidadeInView].media360.hall.splice(fotoIndex, 1);
                }
