...
import { changePrismaProjectFotosGerais } from "@/app/services/changeFotosGerais";import { Fotos360Quarto3Input } from "./Fotos360Quarto3Input";
;
...
export function Fotos360Quarto3({ userData, projectData, openImage, setLoadingFiles, unidadeInView, setFotoType }: FotosGeraisProps) {

    const [editMode, setEditMode] = useState(false)
    const [isDeletingImage, setIsDeletingImage] = useState(false)
    const [deletingImageID, setDeletingImageID] = useState<string>()

    const [destaques, setDestaques] = useState(unidadeInView ? projectData.apartamentTypes[unidadeInView].media360.quarto3 : projectData.apartamentTypes[0].media360.quarto3)

    useEffect(() => {

        if (unidadeInView || unidadeInView == 0) {
            setDestaques(projectData.apartamentTypes[unidadeInView].media360.quarto3)
        }
    ...
    useEffect(() => {
        ...
        const deleteImage = async (imageID: Investment["images"][0]["id"]) => {
            try {
                ...
                const fotoIndex = projectData.apartamentTypes[unidadeInView].media360.quarto3.indexOf(imageToDelete);

                // Remove a foto do array
                if (fotoIndex > -1) {
                    projectData.apartamentTypes[unidadeInView].media360.quarto3.splice(fotoIndex, 1);
                }
