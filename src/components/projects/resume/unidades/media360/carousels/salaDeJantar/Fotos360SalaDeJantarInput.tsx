...
export function Fotos360SalaDeJantarInput({ allowedTypes, accept, projectData, setLoadingFiles, unidadeInView }: FotosInputProps
) {
        ...
        const uploadFotos = async (updateData: Investment, selectedFiles: FileList) => {
            ...
            try {
                ...
                for (let index = 0; index < imageUrls.length; index++) {
                    updateData.apartamentTypes[unidadeInView].media360.salaDeJantar.push(imageUrls[index])
                }

                const response = await changePrismaProjectFotosGerais(projectData.id, updateData)
