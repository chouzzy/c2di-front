...
import { Fotos360SalaDeJantarInput } from "./Fotos360SalaDeJantarInput";
...
export function Fotos360SalaDeJantar({ userData, projectData, openImage, setLoadingFiles, unidadeInView, setFotoType }: FotosGeraisProps) {

    const [editMode, setEditMode] = useState(false)
    const [isDeletingImage, setIsDeletingImage] = useState(false)
    const [deletingImageID, setDeletingImageID] = useState<string>()

    const [destaques, setDestaques] = useState(unidadeInView ? projectData.apartamentTypes[unidadeInView].media360.salaDeJantar : projectData.apartamentTypes[0].media360.salaDeJantar)

    useEffect(() => {

        if (unidadeInView || unidadeInView == 0) {
            setDestaques(projectData.apartamentTypes[unidadeInView].media360.salaDeJantar)
        }
    ...
    useEffect(() => {
        ...
        const deleteImage = async (imageID: Investment["images"][0]["id"]) => {
            try {
                ...
                const fotoIndex = projectData.apartamentTypes[unidadeInView].media360.salaDeJantar.indexOf(imageToDelete);

                // Remove a foto do array
                if (fotoIndex > -1) {
                    projectData.apartamentTypes[unidadeInView].media360.salaDeJantar.splice(fotoIndex, 1);
                }
                ...
                <>
                    ...
                    < Flex w='100%' flexDir={'row'} gap={2} justifyContent={'space-between'} alignItems={'center'} minH={16}>
                        ...
                        <Text fontSize={20} fontWeight={'medium'} mt={2}>
                            Sala de jantar ({destaques.length})
                        </Text>
