...
import { Fotos360LavanderiaInput } from "./Fotos360LavanderiaInput";
...
export function Fotos360Lavanderia({ userData, projectData, openImage, setLoadingFiles, unidadeInView, setFotoType }: FotosGeraisProps) {

    const [editMode, setEditMode] = useState(false)
    const [isDeletingImage, setIsDeletingImage] = useState(false)
    const [deletingImageID, setDeletingImageID] = useState<string>()

    const [destaques, setDestaques] = useState(unidadeInView ? projectData.apartamentTypes[unidadeInView].media360.lavanderia : projectData.apartamentTypes[0].media360.lavanderia)

    useEffect(() => {

        if (unidadeInView || unidadeInView == 0) {
            setDestaques(projectData.apartamentTypes[unidadeInView].media360.lavanderia)
        }
    ...
    useEffect(() => {
        ...
        const deleteImage = async (imageID: Investment["images"][0]["id"]) => {
            try {
                ...
                const fotoIndex = projectData.apartamentTypes[unidadeInView].media360.lavanderia.indexOf(imageToDelete);

                // Remove a foto do array
                if (fotoIndex > -1) {
                    projectData.apartamentTypes[unidadeInView].media360.lavanderia.splice(fotoIndex, 1);
                }
