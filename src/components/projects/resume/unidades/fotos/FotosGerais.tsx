...
import { FotosGeraisInput } from "./FotosGeraisInput";
...
export function FotosGerais({ userData, projectData, openImage, setLoadingFiles, unidadeInView }: FotosGeraisProps) {
    ...
    const [destaques, setDestaques] = useState(unidadeInView ? projectData.apartamentTypes[unidadeInView].fotos : projectData.apartamentTypes[0].fotos)

    useEffect(() => {

        if (unidadeInView || unidadeInView == 0) {
            setDestaques(projectData.apartamentTypes[unidadeInView].fotos)
        }
    ...
    useEffect(() => {
        ...
        const deleteImage = async (imageID: Investment["images"][0]["id"]) => {
            try {
                ...
                console.log('responseImgDeleted')
                console.log(responseImgDeleted)


                // Encontra o índice da foto a ser removida no array fotos
                const fotoIndex = projectData.apartamentTypes[unidadeInView].fotos.indexOf(imageToDelete);

                // Remove a foto do array
                if (fotoIndex > -1) {
                    projectData.apartamentTypes[unidadeInView].fotos.splice(fotoIndex, 1);
                }
