...
interface PersonalDataAndGoalsProps {
    ...
    setTipologies: Dispatch<SetStateAction<TipologiesState[]>>
}
...
export function FourthPage({ tipologies, setTipologies }: PersonalDataAndGoalsProps) {

    const { register: registerOnPageFour, handleSubmit, formState: { errors }, reset } = useForm({});
    ...
    const addTipologyAttributes = async (data: AddTipologyAttributeProps) => {
        ...
        try {
            ...
            setTipologies(prevTipologies => [...(prevTipologies || []), tipology]);
            reset()
            setYupError("")
    ...
    const handleRemoveTipology = (id: string) => {
        setTipologies(tipologies.filter((tipology) => tipology.id !== id));
    }
